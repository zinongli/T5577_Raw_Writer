...
static void t5577_writer_config_item_clicked(void* context, uint32_t index) {
    ...
    if(index == 4) {
        ...
        view_set_previous_callback(byte_input_get_view(app->byte_input), app->config_enter_callback);
...
static void t5577_writer_config_enter_callback(void* context) {
    ...
    app->byte_buffer_item = variable_item_list_add(
        app->variable_item_list_config, edit_block_data_config_label, 1, NULL, NULL);

    FuriString *buffer = furi_string_alloc();
    furi_string_printf(buffer, "%08lX", my_model->content[my_model->edit_block_slc]);
    variable_item_set_current_value_text(
        app->byte_buffer_item, furi_string_get_cstr(buffer));
...
static T5577WriterApp* t5577_writer_app_alloc() {
    ...
    app->variable_item_list_config = variable_item_list_alloc();
    app->config_enter_callback = t5577_writer_config_enter_callback;
